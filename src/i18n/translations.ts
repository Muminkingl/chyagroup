export const translations = {
  en: {
    nav: {
      brand: "Chya Group",
      about: "About Us",
      company: "Our Company",
      vision: "Vision",
      objective: "Objective",
      message: "Message",
      history: "Our History",
      news: "News",
      contact: "Contact Us"
    },
    hero: {
      line1: "Strength in Numbers,",
      line2: "Power in Unity.",
      description: "Connecting markets, empowering industries, and shaping the future of commerce and finance across the region.",
      learnMore: "Learn More",
      contactUs: "Contact Us",
      partners: "Our Partners",
      agentLoop: "Our Agents",
      clients: "Our Clients",
      brandQRs: "Brand QR Codes"
    },
    news: {
      latest: "Latest News",
      all: "All News",
      eyebrow: "Press & Updates",
      headline: "Latest news from",
      headlineAccent: "Chya",
      subtitle: "Stay informed with our latest updates, stories, and achievements driving meaningful impact across the region.",
      viewAll: "View All News",
      readArticle: "Read article"
    },
    features: {
      eyebrow: "OUR SECTORS",
      headline: {
        five: "Five",
        sectors: "sectors,",
        one: "one",
        vision: "vision."
      },
      description: "Chya Group operates across five dynamic industries, spanning 15 branches with 45 professionals driving growth across the region.",
      stats: {
        branches: "Offices",
        professionals: "Professionals",
        vision: "Unified Vision"
      },
      button: "Our Journey",
      quote: {
        line1: "Diverse expertise.",
        line2: "Unified purpose.",
        line3: "Lasting impact."
      },
      items: [
        {
          id: 'general-trading',
          tag: 'General Trading',
          cardTitle: 'GENERAL TRADING',
          contentLabel: 'GENERAL TRADING',
          contentTitle: 'Powering commerce across the region',
          contentBody: 'Our general trading arm spans a diverse product portfolio, supplying markets across the region with reliability and scale. With 14 branches and deep supplier networks, we keep commerce moving.',
          metrics: [
            { label: 'Branches', value: '14' },
            { label: 'Employees', value: '44' },
            { label: 'Companies', value: '3' }
          ]
        },
        {
          id: 'money-exchange',
          tag: 'Money Exchange',
          cardTitle: 'MONEY EXCHANGE & FINANCIAL SERVICE',
          contentLabel: 'FINANCIAL SERVICES',
          contentTitle: 'Trusted financial exchange & services',
          contentBody: 'Chya Group operates 5 dedicated money exchange offices delivering fast, secure, and competitive currency exchange alongside comprehensive financial services for individuals and businesses.',
          metrics: [
            { label: 'Offices', value: '5' },
            { label: 'Currencies', value: '20+' },
            { label: 'Daily Ops', value: '24/7' }
          ]
        },
        {
          id: 'mobile-tech',
          tag: 'Mobile & Tech',
          cardTitle: 'MOBILE & TECHNOLOGY',
          contentLabel: 'MOBILE & TECHNOLOGY',
          contentTitle: 'Connecting people through technology',
          contentBody: 'From mobile devices to digital solutions, our technology division serves consumers and enterprises alike. We bridge the gap between cutting-edge tech and everyday needs across the region.',
          metrics: [
            { label: 'Online Ops', value: '3' },
            { label: 'Partners', value: '10+' },
            { label: 'Support', value: '24/7' }
          ]
        },
        {
          id: 'printing',
          tag: 'Printing',
          cardTitle: 'PRINTING & PHOTOCOPY',
          contentLabel: 'PRINTING & PHOTOCOPY',
          contentTitle: 'Professional printing for every need',
          contentBody: 'Our printing and photocopy center delivers high-quality print solutions for businesses, institutions, and individuals. From documents to banners, we handle it all with precision and speed.',
          metrics: [
            { label: 'Office', value: '1' },
            { label: 'Formats', value: 'All' },
            { label: 'Turnaround', value: 'Fast' }
          ]
        },
        {
          id: 'online-trading',
          tag: 'Online Trading',
          cardTitle: 'ONLINE TRADING',
          contentLabel: 'ONLINE TRADING',
          contentTitle: 'Smart trading in digital markets',
          contentBody: 'Our online trading division operates 3 active digital market channels, giving clients access to global financial instruments. Backed by 14 bank & company partnerships for seamless execution.',
          metrics: [
            { label: 'Channels', value: '3' },
            { label: 'Bank Partners', value: '14' },
            { label: 'Uptime', value: '99.9%' }
          ]
        }
      ]
    },
    timelineSectors: {
      eyebrow: "OUR SECTORS",
      headline: {
        diverse: "Diverse Strengths,",
        unified: "Unified",
        vision: "Vision."
      },
      subtitle: "Chya Group operates across five dynamic sectors, delivering trusted solutions and creating value in every direction we serve.",
      button: "Explore More",
      sectors: [
        {
          id: 'general-trading',
          title: "GENERAL TRADING",
          desc: "Our general trading arm spans a diverse product portfolio, supplying markets across the region with quality and reliability.",
          items: ['Lammat Al marjan', 'Chyay Mateen', 'Chya amazon']
        },
        {
          id: 'money-exchange',
          title: "MONEY EXCHANGE &\nFINANCIAL SERVICE",
          desc: "Chya Group operates a dedicated money exchange division delivering fast, secure, and competitive financial services.",
          items: ['Khaki Sarwar', 'Chya', 'Chya gold', 'Lutkay chya', 'Barzy chya', 'Hangaw dibaga station']
        },
        {
          id: 'mobile-tech',
          title: "MOBILE &\nTECHNOLOGY",
          desc: "From mobile devices to digital solutions, our technology division powers consumers and enterprises alike.",
          items: ['Chya phone', 'Chya tech']
        },
        {
          id: 'printing',
          title: "PRINTING &\nPHOTOCOPY",
          desc: "Our printing and photocopy center delivers high-quality print solutions for businesses, institutions, and individuals.",
          items: ['Blue printing']
        },
        {
          id: 'online-trading',
          title: "ONLINE TRADING",
          desc: "Our online trading division operates 3 active digital market channels, giving clients access to global financial opportunities.",
          items: ['Chya travel', 'Kiva luxury']
        }
      ]
    },
    history: {
      eyebrow: "Our Legacy",
      title: "Chya Group",
      since: "since 2019",
      description: "Shaping modern horizons with unparalleled dedication. Discover how our vision evolved into a foundation for tomorrow's infrastructure.",
      readMore: "Read More"
    },
    agents: {
      eyebrow: "AUTHORIZED AGENTS",
      headline: {
        excellence: "Excellence in growth,",
        united: "united by",
        trust: "trust."
      },
      subtitle: "We are the authorized agents for leading financial and technology brands, delivering excellence and driving success across the region."
    },
    clients: {
      eyebrow: "OUR CLIENTS",
      headline: {
        trusted: "Trusted by Leading Brands",
        worldwide: "Worldwide."
      },
      subtitle: "We are proud to work with a diverse range of clients across industries. Their trust inspires us to deliver excellence every day."
    },
    brandQRs: {
      eyebrow: "FOLLOW OUR BRANDS",
      title: "Official Brand Channels",
      description: "Scan to explore our brands and stay connected across our diverse business sectors.",
      scanToFollow: "SCAN TO FOLLOW",
      banner: {
        title: "One Group. Many Solutions.",
        subtitle: "Each brand under the Chya Group umbrella is dedicated to delivering excellence in its field.",
        stats: {
          sectors: "5",
          offices: "15",
          professionals: "45",
          vision: "1",
          visionLabel: "Unified Vision",
          sectorsLabel: "Sectors",
          officesLabel: "Offices",
          professionalsLabel: "Professionals"
        }
      }
    },
    location: {
      eyebrow: "Global HQ",
      title: "Visit our headquarters",
      description: "Strategically located in Erbil. Come experience the future of commerce and industry in person.",
      hqTitle: "Erbil Headquarters",
      hqAddress: "Runaki Street, Erbil 44001, Kurdistan Region, Iraq",
      email: "Email",
      phone: "Phone",
      contactBtn: "Contact Us"
    },
    footer: {
      description: "Pioneering excellence across diversified investments. Building a sustainable and innovative future for the region.",
      company: "Company",
      sectors: "Sectors",
      links: {
        about: "About Us",
        teams: "Teams",
        history: "History",
        contact: "Contact Us"
      },
      copyright: "All rights reserved.",
      followUs: "Follow Us",
      address: "Erbil, Kurdistan Region, Iraq"
    },
    contact: {
      title: "Let's start a conversation",
      description: "Whether you have a question about our services, or anything else, our team is ready to answer all your questions.",
      form: {
        fullName: "Full Name",
        email: "Email Address",
        message: "Message",
        placeholderName: "Full Name",
        placeholderEmail: "Email Address",
        placeholderMessage: "How can we help you?",
        submit: "Send Message",
        sending: "Sending...",
        privacy: "By submitting, you agree to our privacy policy.",
        successTitle: "Message Received",
        successDesc: "Thank you, {name}! We've received your message and will get back to you shortly.",
        redirect: "Redirecting to home in {seconds}s"
      },
      info: {
        title: "Contact Information",
        emailTitle: "Email Us",
        visitTitle: "Visit Headquarters",
        callTitle: "Call Us",
        hqErbil: "HQ Erbil"
      }
    },
    newsArchive: {
      title: "News &",
      insights: "Insights",
      description: "Official news, announcements, and updates from Chya Group — covering our expansions, partnerships, and corporate milestones.",
      latest: "Latest News",
      allNews: "All News",
      noPosts: "No news posts published yet. Please check back later.",
      readTime: "{minutes} min read",
      author: "Chya Group"
    },
    about: {
      hero: {
        eyebrow: "Discover Our Roots",
        title: "About Us",
        description: "Driven by vision, guided by experience. We are a collective of industry leaders dedicated to shaping the future of global enterprise.",
        buttonText: "Discover Our Journey"
      },
      nav: {
        title: "On this page",
        leadership: "Ownership Team",
        history: "Our History"
      },
      team: {
        title: "Ownership Team",
        description: "Meet the visionary leadership driving our strategic direction and operational excellence.",
        members: {
          haji: "Haji Muzafer Al Sarraf",
          hajiRole: "Founder & Honorary Chairman",
          ibrahim: "Ibrahim Haji Muzafer",
          ibrahimRole: "Board Chairman & General Manager",
          bashir: "Bashir Al-Rajih",
          bashirRole: "Board Member & CEO",
          ali: "Ali Nuaman Haddad",
          aliRole: "Board Member & Operations Manager",
          qasim: "Qasim Ali Othman",
          qasimRole: "Management Manager"
        }
      },
      history: {
        title: "Our History",
        summary: "Chya Group: Established in 2019, serving for 7 years in Erbil across 5 diverse sectors (General Trading, Money Exchange & Financial Services, Mobile & Technology, Printing & Photocopy, Online Trading). Our team consists of 44 members, owning 3 companies, 5 money exchange offices, 1 printing office, 1 salary disbursement center, 1 project, 1 exhibition, 3 online businesses, and serving as agents for 14 banks and companies.",
        explore: "Click to explore",
        timeline: {
          foundation: {
            title: "Lamaat Al Marjan Company",
            desc: "General Trading - Construction materials (Wood, MDF, Furniture, Steel, Cement, Concrete).",
            content: "Lamaat Al Marjan for General Trading was the first company under the Chya Group general trading sector. Founded on 2019/11/30, it specializes in construction materials including Wood, MDF, Furniture, Steel, and Concrete."
          },
          mateen: {
            title: "Chyay Matin Company",
            desc: "General Trading - Electronics, Household items, Clothing.",
            content: "Chyay Matin for General Trading is the second company in the Chya Group general trading sector. Founded on 2026/03/01, it deals in electronic devices, household appliances, clothing, and second-hand goods."
          },
          blueprint: {
            title: "Blue Printing Office",
            desc: "Printing & Photocopy - Passports, National cards, document services.",
            content: "Blue Printing for Printing and Photocopying was established on 2025/10/19. It specializes in printing, photography, managing passport and national card fees, and international passport appointment booking."
          },
          exchange: {
            title: "Chya Exchange Office",
            desc: "Currency Exchange - Remittances and bank account transfers.",
            content: "Chya Office for Money Exchange was the first branch in the financial services sector. Established on 2021/06/14 in Erbil's Old Bourse, it handles all types of currency exchange and global money transfers via cash and bank accounts."
          },
          gold: {
            title: "Chya Gold Office",
            desc: "International Exchange - Silopi, Turkey branch.",
            content: "Chya Gold for Money Exchange was the second financial branch. Established on 2023/08/28 in Silopi, Turkey, it manages currency exchange and transfers to all cities and bank accounts within Turkey."
          },
          hangaw: {
            title: "Hangaw Exchange Office",
            desc: "Financial Services - Runaki Street branch.",
            content: "Hangaw Office for Money Exchange was the third financial branch. Established on 2024/03/21 on Runaki Street, Erbil, specializing in global remittances and local/international card funding."
          },
          khaki: {
            title: "Khaki Sarwar Company",
            desc: "Currency Exchange - Pirmam Road branch.",
            content: "Khaki Sarwar for Money Exchange was the fourth financial branch. Established on 2025/02/16 in Masif Road inside BM2 Petrol Station, providing comprehensive currency and remittance services."
          },
          lutkay: {
            title: "Lutkay Chya Office",
            desc: "Financial Services - Ankawa branch.",
            content: "Lutkay Chya for Money Exchange was the fifth financial branch. Established on 2024/10/20 in Ankawa district, Erbil, simplifying global money transfers and local currency exchange."
          },
          barzy: {
            title: "Barzy Chya Office",
            desc: "Financial Services - Soran city branch.",
            content: "Barzy Chya for Money Exchange was the sixth financial branch. Established on 2025/03/06 in Soran city center, serving the region with reliable remittances and exchange services."
          },
          dibaga: {
            title: "Hangaw Dibaga Station",
            desc: "Salary Disbursement - Military and Civil services.",
            content: "Hangaw Dibaga for Salary Disbursement launched on 2025/08/01 inside Hangaw Office. It manages military and civil salaries, pensions, and services for Qi Card, Super Qi, and Master Card."
          },
          amazon: {
            title: "Chya Amazon Project",
            desc: "E-commerce - Second-hand goods (Retail & Wholesale).",
            content: "Chya Amazon for second-hand goods (Retail/Wholesale) is part of our trading sector. Founded on 2026/03/01 in Rawanduz, Soran, it operates on a fair and transparent trade system."
          },
          phone: {
            title: "Chya Phone Exhibition",
            desc: "Technology - Mobile, iPads, and Laptops.",
            content: "Chya Phone for mobile trading launched on 2026/03/24 in Soran city center. It specializes in phones, iPads, laptops, computers, electronic devices, and accessories."
          },
          travel: {
            title: "Chya Travel",
            desc: "Services - Tickets, Visas, and Hotel bookings.",
            content: "Chya Travel for tickets and visas opened on 2020/01/22. Services include flight tickets, tourism visas, hotel and car bookings, and medical guide/translation services."
          },
          tech: {
            title: "Chya Tech",
            desc: "IT Solutions - Systems, accounting, and software.",
            content: "Chya Tech for systems and computers launched on 2025/08/11. It provides accounting systems, statistics, archiving, HR software, and electronic hardware sales."
          },
          luxury: {
            title: "Kiva Luxury",
            desc: "Lifestyle - Jewellery, watches, and perfumes.",
            content: "Kiva Luxury for jewelry and accessories launched on 2022/01/20, specializing in premium jewelry, watches, and international perfume brands."
          }
        }
      }
    }
  },
  ar: {
    nav: {
      brand: "مجموعة چیا",
      about: "من نحن",
      company: "شركتنا",
      vision: "رؤيتنا",
      objective: "أهدافنا",
      message: "رسالتنا",
      history: "تاريخنا",
      news: "الأخبار",
      contact: "اتصل بنا"
    },
    hero: {
      line1: "القوة في الأرقام،",
      line2: "والقدرة في الاتحاد.",
      description: "ربط الأسواق، تمكين الصناعات، وتشكيل مستقبل التجارة والتمويل في جميع أنحاء المنطقة.",
      learnMore: "تعرف على المزيد",
      contactUs: "اتصل بنا",
      partners: "شركاؤنا",
      agentLoop: "وكالاتنا",
      clients: "عملاؤنا",
      brandQRs: "رموز QR للعلامات التجارية"
    },
    news: {
      latest: "أحدث الأخبار",
      all: "كل الأخبار",
      eyebrow: "آخر الأخبار",
      headline: "أحدث الأخبار من",
      headlineAccent: "چیا",
      subtitle: "ابقَ على اطلاع بآخر أخبارنا وإنجازاتنا التي تحدث تأثيراً حقيقياً في المنطقة.",
      viewAll: "عرض كل الأخبار",
      readArticle: "اقرأ المقال"
    },
    features: {
      eyebrow: "قطاعاتنا",
      headline: {
        five: "خمسة",
        sectors: "قطاعات،",
        one: "رؤية",
        vision: "واحدة."
      },
      description: "تعمل مجموعة چیا عبر خمسة سێکتەرات ديناميكية، وتضم ١٥ مكتباً مع ٤٥ محترفاً يدفعون عجلة النمو في جميع أنحاء المنطقة.",
      stats: {
        branches: "مكاتبنا",
        professionals: "محترفونا",
        vision: "رؤية موحدة"
      },
      button: "رحلتنا",
      quote: {
        line1: "خبرات متنوعة.",
        line2: "هدف موحد.",
        line3: "تأثير دائم."
      },
      items: [
        {
          id: 'general-trading',
          tag: 'تجارة عامة',
          cardTitle: 'التجارة العامة',
          contentLabel: 'التجارة العامة',
          contentTitle: 'دعم التجارة في جميع أنحاء المنطقة',
          contentBody: 'تمتد تجاراتنا العامة عبر مجموعة متنوعة من المنتجات، حيث نوفر للأسواق في جميع أنحاء المنطقة الموثوقية والحجم. مع 14 فرعاً وشبكات موردين عميقة، نحافظ على حركة التجارة.',
          metrics: [
            { label: 'الفروع', value: '14' },
            { label: 'الموظفون', value: '44' },
            { label: 'الشركات', value: '3' }
          ]
        },
        {
          id: 'money-exchange',
          tag: 'صرافة',
          cardTitle: 'الصرافة والخدمات المالية',
          contentLabel: 'الخدمات المالية',
          contentTitle: 'صرافة وخدمات مالية موثوقة',
          contentBody: 'تدير مجموعة چیا 5 مكاتب صرافة مخصصة لتقديم صرف عمالت سريع وآمن وتنافسي إلى جانب خدمات مالية شاملة للأفراد والشركات.',
          metrics: [
            { label: 'المكاتب', value: '5' },
            { label: 'العملات', value: '20+' },
            { label: 'العمليات', value: '24/7' }
          ]
        },
        {
          id: 'mobile-tech',
          tag: 'موبايل وتكنولوجيا',
          cardTitle: 'الموبايل والتكنولوجيا',
          contentLabel: 'الموبايل والتكنولوجيا',
          contentTitle: 'ربط الناس من خلال التكنولوجيا',
          contentBody: 'من الأجهزة المحمولة إلى الحلول الرقمية، يخدم قسم التكنولوجيا لدينا المستهلكين والمؤسسات على حد سواء. نحن نسد الفجوة بين التكنولوجيا المتقدمة والاحتياجات اليومية.',
          metrics: [
            { label: 'عمليات أونلاين', value: '3' },
            { label: 'الشركاء', value: '10+' },
            { label: 'الدعم', value: '24/7' }
          ]
        },
        {
          id: 'printing',
          tag: 'طباعة',
          cardTitle: 'الطباعة والاستنساخ',
          contentLabel: 'الطباعة والاستنساخ',
          contentTitle: 'طباعة احترافية لكل الاحتياجات',
          contentBody: 'يقدم مركز الطباعة والاستنساخ لدينا حلول طباعة عالية الجودة للشركات والمؤسسات والأفراد. من المستندات إلى اللافتات، نتعامل مع كل شيء بدقة وسرعة.',
          metrics: [
            { label: 'المكتب', value: '1' },
            { label: 'التنسيقات', value: 'الكل' },
            { label: 'التنفيذ', value: 'سريع' }
          ]
        },
        {
          id: 'online-trading',
          tag: 'تداول أونلاين',
          cardTitle: 'التداول عبر الإنترنت',
          contentLabel: 'التداول عبر الإنترنت',
          contentTitle: 'تداول ذكي في الأسواق الرقمية',
          contentBody: 'يدير قسم التداول عبر الإنترنت 3 قنوات سوق رقمية نشطة، مما يتيح للعملاء الوصول إلى الأدوات المالية العالمية والشركات.',
          metrics: [
            { label: 'القنوات', value: '3' },
            { label: 'شركاء بنكيون', value: '14' },
            { label: 'التواجد', value: '99.9%' }
          ]
        }
      ]
    },
    timelineSectors: {
      eyebrow: "قطاعاتنا",
      headline: {
        diverse: "قدرات متنوعة،",
        unified: "رؤية",
        vision: "موحدة."
      },
      subtitle: "تعمل مجموعة چیا عبر خمسة قطاعات ديناميكية، وتقدم حلولاً موثوقة وتخلق قيمة في كل اتجاه نخدمه.",
      button: "استكشف المزيد",
      sectors: [
        {
          id: 'general-trading',
          title: "التجارة العامة",
          desc: "يغطي ذراعنا للتجارة العامة مجموعة متنوعة من المنتجات، ويزود الأسواق في جميع أنحاء المنطقة بالجودة والموثوقية.",
          items: ['لمة المرجان', 'چياي متين', 'چیا أمازون']
        },
        {
          id: 'money-exchange',
          title: "الصرافة والخدمات المالية",
          desc: "تدير مجموعة چیا قسماً مخصصاً للصرافة يقدم خدمات مالية سريعة وآمنة وتنافسية.",
          items: ['خاكي سرور', 'چیا', 'چیا گۆڵد', 'لوتکەی چیا', 'بەرزى چیا', 'منفذ ديکبە هەنگاو']
        },
        {
          id: 'mobile-tech',
          title: "الموبايل والتكنولوجيا",
          desc: "من الأجهزة المحمولة إلى الحلول الرقمية، يدعم قسم التكنولوجيا لدينا المستهلكين والشركات على حد سواء.",
          items: ['چیا فۆن', 'چیا تێك']
        },
        {
          id: 'printing',
          title: "الطباعة والتصوير",
          desc: "يوفر مركز الطباعة والتصوير لدينا حلول طباعة عالية الجودة للشركات والمؤسسات والأفراد.",
          items: ['بلو برێنتینگ']
        },
        {
          id: 'online-trading',
          title: "التجارة عبر الإنترنت",
          desc: "يدير قسم التجارة عبر الإنترنت لدينا 3 قنوات نشطة للسوق الرقمي، مما يتيح للعملاء الوصول إلى الفرص المالية العالمية.",
          items: ['چیا تڕاڤڵ', 'کیڤا لوکژوری']
        }
      ]
    },
    history: {
      eyebrow: "تراثنا",
      title: "مجموعة چیا",
      since: "منذ عام 2019",
      description: "صياغة آفاق حديثة بتفانٍ لا مثيل له. اكتشف كيف تطورت رؤيتنا لتصبح أساساً للبنية التحتية للغد.",
      readMore: "اقرأ المزيد"
    },
    agents: {
      eyebrow: "الوكالات المعتمدة",
      headline: {
        excellence: "التميز في النمو،",
        united: "يجمعه",
        trust: "الثقة."
      },
      subtitle: "نحن الوكلاء المعتمدون للعلامات التجارية الرائدة في مجالات التمويل والتكنولوجيا، ونقدم التميز وندفع النجاح في جميع أنحاء المنطقة."
    },
    clients: {
      eyebrow: "عملاؤنا",
      headline: {
        trusted: "موثوقون من قبل العلامات التجارية",
        worldwide: "الرائدة عالمياً."
      },
      subtitle: "نحن فخورون بالعمل مع مجموعة متنوعة من العملاء عبر الصناعات. ثقتهم تلهمنا لتقديم التميز كل يوم."
    },
    brandQRs: {
      eyebrow: "تابع برانداتنا",
      title: "قنوات البراندات الرسمية",
      description: "امسح الرمز لاستكشاف برانداتنا والبقاء على اتصال عبر قطاعات أعمالنا المتنوعة.",
      scanToFollow: "امسح للمتابعة",
      banner: {
        title: "مجموعة واحدة. حلول عديدة.",
        subtitle: "كل براند تحت مظلة مجموعة چیا مكرس لتقديم التميز في مجاله.",
        stats: {
          sectors: "٥",
          offices: "١٥",
          professionals: "٤٥",
          vision: "١",
          visionLabel: "رؤية موحدة",
          sectorsLabel: "قطاعات",
          officesLabel: "مكاتبنا",
          professionalsLabel: "محترفونا"
        }
      }
    },
    location: {
      eyebrow: "المقر العالمي",
      title: "تفضل بزيارة مقرنا الرئيسي",
      description: "موقع استراتيجي في أربيل. تعال وجرب مستقبل التجارة والصناعة شخصياً.",
      hqTitle: "مقر أربيل الرئيسي",
      hqAddress: "شارع رونكي ، أربيل 44001 ، إقليم كردستان ، العراق",
      email: "البريد الإلكتروني",
      phone: "الهاتف",
      contactBtn: "اتصل بنا"
    },
    footer: {
      description: "الريادة في التميز عبر الاستثمارات المتنوعة. بناء مستقبل مستدام ومبتكر للمنطقة.",
      company: "الشركة",
      sectors: "القطاعات",
      links: {
        about: "من نحن",
        teams: "فريق العمل",
        history: "تاريخنا",
        contact: "اتصل بنا"
      },
      copyright: "جميع الحقوق محفوظة.",
      followUs: "تابعونا",
      address: "أربيل، إقليم كردستان، العراق"
    },
    contact: {
      title: "لنبدأ محادثة",
      description: "سواء كان لديك سؤال حول خدماتنا، أو أي شيء آخر، فإن فريقنا مستعد للإجابة على جميع أسئلتك.",
      form: {
        fullName: "الاسم الكامل",
        email: "البريد الإلكتروني",
        message: "الرسالة",
        placeholderName: "الاسم الكامل",
        placeholderEmail: "البريد الإلكتروني",
        placeholderMessage: "كيف يمكننا مساعدتك؟",
        submit: "إرسال الرسالة",
        sending: "جاري الإرسال...",
        privacy: "من خلال الإرسال، فإنك توافق على سياسة الخصوصية الخاصة بنا.",
        successTitle: "تم استلام الرسالة",
        successDesc: "شكراً لك، {name}! لقد استلمنا رسالتك وسنعاود الاتصال بك قريباً.",
        redirect: "إعادة التوجيه إلى الصفحة الرئيسية خلال {seconds} ثانية"
      },
      info: {
        title: "معلومات الاتصال",
        emailTitle: "راسلنا",
        visitTitle: "زوروا مقرنا",
        callTitle: "اتصل بنا",
        hqErbil: "مقر أربيل"
      }
    },
    newsArchive: {
      title: "الأخبار و",
      insights: "الرؤى",
      description: "الأخبار الرسمية والإعلانات والتحديثات من مجموعة چیا - تغطي توسعاتنا وشراكاتنا وإنجازاتنا المؤسسية.",
      latest: "أحدث الأخبار",
      allNews: "كل الأخبار",
      noPosts: "لم يتم نشر أي أخبار بعد. يرجى العودة لاحقاً.",
      readTime: "قراءة {minutes} دقائق",
      author: "مجموعة چیا"
    },
    about: {
      hero: {
        eyebrow: "اكتشف جذورنا",
        title: "من نحن",
        description: "مدفوعون بالرؤية، ومسترشدون بالخبرة. نحن مجموعة من قادة الصناعة المكرسين لتشكيل مستقبل المشاريع العالمية.",
        buttonText: "اكتشف رحلتنا"
      },
      nav: {
        title: "في هذه الصفحة",
        leadership: "فريق الملكية",
        history: "تاريخنا"
      },
      team: {
        title: "فريق الملكية",
        description: "تعرف على القيادة الرؤيوية التي تقود توجهنا الاستراتيجي وتميزنا التشغيلي.",
        members: {
          haji: "حجي مظفر الصراف",
          hajiRole: "المؤسس ورئيس الشرف",
          ibrahim: "إبراهيم حجي مظفر",
          ibrahimRole: "رئيس مجلس الإدارة والمدير العام",
          bashir: "بشير الراجح",
          bashirRole: "عضو مجلس الإدارة والمدير التنفيذي",
          ali: "علي نعمان حداد",
          aliRole: "عضو مجلس الإدارة ومدير العمليات",
          qasim: "قاسم علي عثمان",
          qasimRole: "مدير الإدارة"
        }
      },
      history: {
        title: "تاريخنا",
        summary: "مجموعة چیا: تأسست عام 2019 وتخدم منذ 7 سنوات في أربيل عبر 5 قطاعات متنوعة (التجارة العامة، الصيرفة والخدمات المالية، الموبايل والتكنولوجيا، الطباعة والاستنساخ، التجارة الإلكترونية). يتكون فريقنا من 44 عضواً، ونمتلك 3 شركات، و5 مكاتب صيرفة، ومكتب طباعة واحد، ومركز صرف رواتب واحد، ومشروع واحد، ومعرض واحد، و3 أعمال إلكترونية، ونعمل كوكلاء لـ 14 بنكاً وشركة.",
        explore: "انقر للاكتشاف",
        timeline: {
          foundation: {
            title: "شركة لمعة المرجان",
            desc: "تجارة عامة - مواد إنشائية (خشب، MDF، أثاث، فولاذ، إسمنت، كونكريت).",
            content: "كانت شركة لمعة المرجان للتجارة العامة أول شركة تابعة لقطاع التجارة العامة في مجموعة چیا. تأسست في 30/11/2019، وهي متخصصة في مواد البناء بما في ذلك الخشب والـ MDF والأثاث والحديد والخرسانة."
          },
          mateen: {
            title: "شركة چيای مەتین",
            desc: "تجارة عامة - إلكترونيات، مستلزمات منزلية، ملابس.",
            content: "شركة چيای مەتین للتجارة العامة هي الشركة الثانية في قطاع التجارة العامة لمجموعة چیا. تأسست في 01/03/2026، وتعمل في الأجهزة الإلكترونية والأدوات المنزلية والملابس والسلع المستعملة."
          },
          blueprint: {
            title: "مكتب بلو برينتينغ",
            desc: "الطباعة والاستنساخ - الجوازات، البطاقات الوطنية، خدمات الوثائق.",
            content: "تأسس مكتب بلو برينتينغ للطباعة والاستنساخ في 19/10/2025. وهو متخصص في الطباعة والتصوير وإدارة رسوم الجوازات والبطاقات الوطنية وحجز مواعيد الجوازات الدولية."
          },
          exchange: {
            title: "مكتب چيا للصيرفة",
            desc: "تصريف العملات - حوالات وتحويلات حسابات بنكية.",
            content: "كان مكتب چيا للصيرفة هو الفرع الأول في قطاع الخدمات المالية. تأسس في 14/06/2021 في بورصة أربيل القديمة، ويتعامل مع جميع أنواع تصريف العملات وتحويل الأموال العالمي نقداً وعبر الحسابات البنكية."
          },
          gold: {
            title: "مكتب چيا غولد للصيرفة",
            desc: "صيرفة دولية - فرع سلوبي، تركيا.",
            content: "كان مكتب چيا غولد للصيرفة هو الفرع المالي الثاني. تأسس في 28/08/2023 في سلوبي، تركيا، ويدير تصريف العملات والتحويلات إلى جميع المدن والحسابات البنكية داخل تركيا."
          },
          hangaw: {
            title: "مكتب هەنگاو للصيرفة",
            desc: "خدمات مالية - فرع شارع روناکی.",
            content: "مكتب هەنگاو للصيرفة هو الفرع المالي الثالث. تأسس في 21/03/2024 في شارع روناکی، أربيل، وهو متخصص في الحوالات العالمية وتمويل البطاقات المحلية والدولية."
          },
          khaki: {
            title: "شركة خاکی سەروەر",
            desc: "تصريف العملات - فرع طريق بيرمام.",
            content: "شركة خاکی سەروەر للصيرفة هي الفرع المالي الرابع. تأسست في 16/02/2025 في طريق مصيف داخل محطة وقود BM2، وتقدم خدمات شاملة للعملات والحوالات."
          },
          lutkay: {
            title: "مكتب لوتکەی چیا",
            desc: "خدمات مالية - فرع عنكاوا.",
            content: "مكتب لوتکەی چيا للصيرفة هو الفرع المالي الخامس. تأسس في 20/10/2024 في منطقة عنكاوا، أربيل، لتسهيل تحويلات الأموال العالمية وتصريف العملات المحلية."
          },
          barzy: {
            title: "مكتب بـەرزی چیا",
            desc: "خدمات مالية - فرع مدينة سوران.",
            content: "مكتب بـەرزی چيا للصيرفة هو الفرع المالي السادس. تأسس في 06/03/2025 في مركز مدينة سوران، لخدمة المنطقة بحوالات وخدمات صيرفة موثوقة."
          },
          dibaga: {
            title: "منفذ ديبكة هەنگاو",
            desc: "صرف الرواتب - خدمات عسكرية ومدنية.",
            content: "انطلق منفذ ديبكة هەنگاو لصرف الرواتب في 01/08/2025 داخل مكتب هەنگاو. ويدير الرواتب العسكرية والمدنية والمعاشات وخدمات كي كارد، سوبر كي، وماستر كارد."
          },
          amazon: {
            title: "مشروع چيا أمازون",
            desc: "تجارة إلكترونية - سلع مستعملة (بالجملة والمفرد).",
            content: "مشروع چيا أمازون للسلع المستعملة (Retail/Wholesale) هو جزء من قطاعنا التجاري. تأسس في 01/03/2026 في رواندوز، سوران، ويعمل بنظام تجاري عادل وشفاف."
          },
          phone: {
            title: "معرض چيا فون",
            desc: "تكنولوجيا - موبايل، آيباد، ولابتوب.",
            content: "انطلق معرض چيا فون لتجارة الموبايلات في 24/03/2026 في مركز مدينة سوران. وهو متخصص في الموبايلات والآيبادات واللابتوبات وأجهزة الكمبيوتر والأجهزة الإلكترونية والإكسسوارات."
          },
          travel: {
            title: "چيا ترافيل",
            desc: "خدمات - تذاكر، فيزا، وحجز فنادق.",
            content: "افتتح مكتب چيا ترافيل للتذاكر والفيزا في 22/01/2020. وتتضمن الخدمات تذاكر الطيران، فيزا السياحة، حجز الفنادق والسيارات، وخدمات الدليل الطبي والترجمة."
          },
          tech: {
            title: "چيا تيك",
            desc: "حلول تقنية - أنظمة، محاسبة، وبرمجيات.",
            content: "انطلقت شركة چيا تيك للأنظمة والكمبيوتر في 11/08/2025. وتوفر أنظمة المحاسبة والإحصاء والأرشفة وبرمجيات الموارد البشرية ومبيعات الأجهزة الإلكترونية."
          },
          luxury: {
            title: "كيفا لوجري",
            desc: "نمط حياة - مجوهرات، ساعات، وعطور.",
            content: "انطلقت شركة كيفا لوجري للمجوهرات والإكسسوارات في 20/01/2022، وهي متخصصة في المجوهرات والساعات الفاخرة والعطور العالمية."
          }
        }
      }
    }
  },
  ku: {
    nav: {
      brand: "چیا گرووپ",
      about: "دەربارەی ئێمە",
      company: "کۆمپانیاکەمان",
      vision: "دیدگامان",
      objective: "ئامانجمان",
      message: "پەیاممان",
      history: "مێژووەکەمان",
      news: "هەواڵەکان",
      contact: "پەیوەندیمان پێوە بکەن"
    },
    hero: {
      line1: "هێز لە ژمارەدا،",
      line2: "دەسەڵات لە یەکڕیزیدا.",
      description: "بەستنەوەی بازاڕەکان، بەهێزکردنی پیشەسازییەکان، و داڕشتنی داهاتووی بازرگانی و دارایی لە سەرانسەری ناوچەکەدا.",
      learnMore: "زیاتر بزانە",
      contactUs: "پەیوەندیمان پێوە بکەن",
      partners: "هاوبەشەکانمان",
      agentLoop: "وەکالەکان",
      clients: "کڕیارەکانمان",
      brandQRs: "کۆدەکانی QRی براندەکان"
    },
    news: {
      latest: "دوایین هەواڵەکان",
      all: "هەموو هەواڵەکان",
      eyebrow: "هەواڵ و نوێکارییەکان",
      headline: "دوایین هەواڵەکانی",
      headlineAccent: "چیا گرووپ",
      subtitle: "لەگەڵ دوایین نوێکارییەکان و دەستکەوتەکانمان بمێنەوە کە کاریگەری واقیعیان لە ناوچەکەدا هەیە.",
      viewAll: "بینینی هەموو هەواڵەکان",
      readArticle: "خوێندنەوەی بابەت"
    },
    features: {
      eyebrow: "سێکتەرەکانمان",
      headline: {
        five: "پێنج",
        sectors: "سێکتەر،",
        one: "یەک",
        vision: "دیدگا."
      },
      description: "چیا گرووپ لە پێنج سێکتەری جیاوازدا کاردەکات، کە ١٥ نووسینگە و ٤٥ کەسی پسپۆڕ کاردەکەن بۆ گەشەپێدانی ناوچەکە.",
      stats: {
        branches: "نووسینگە",
        professionals: "پسپۆڕ",
        vision: "دیدگای یەکگرتوو"
      },
      button: "گەشتەکەمان",
      quote: {
        line1: "پسپۆڕی جۆراوجۆر.",
        line2: "ئامانجی یەکگرتوو.",
        line3: "کاریگەری بەردەوام."
      },
      items: [
        {
          id: 'general-trading',
          tag: 'بازرگانی گشتی',
          cardTitle: 'بازرگانی گشتی',
          contentLabel: 'بازرگانی گشتی',
          contentTitle: 'بەهێزکردنی بازرگانی لە ناوچەکەدا',
          contentBody: 'بەشی بازرگانی گشتی ئێمە چەندین جۆری بەرهەم لەخۆدەگرێت، کە بازاڕەکانی ناوچەکە دابین دەکات. بە 14 لق و تۆڕێکی بەهێزی دابینکەران، بازرگانی بەردەوام دەکەین.',
          metrics: [
            { label: 'لقەکان', value: '14' },
            { label: 'کارمەندان', value: '44' },
            { label: 'کۆمپانیاکان', value: '3' }
          ]
        },
        {
          id: 'money-exchange',
          tag: 'ئاڵوگۆڕی دراو و سێرڤسی دارایی',
          cardTitle: 'ئاڵوگۆڕی دراو و سێرڤسی دارایی',
          contentLabel: 'خزمەتگوزارییە داراییەکان',
          contentTitle: 'گۆڕینەوە و خزمەتگوزاری دارایی جێی متمانە',
          contentBody: 'چیا گرووپ 5 نووسینگەی تایبەت بە گۆڕینەوەی دراو بەڕێوەدەبات کە خزمەتگوزاری خێرا و سەلامەت پێشکەش بە تاک و کۆمپانیاکان دەکەن.',
          metrics: [
            { label: 'نووسینگەکان', value: '5' },
            { label: 'دراوەکان', value: '20+' },
            { label: 'کارکردن', value: '24/7' }
          ]
        },
        {
          id: 'mobile-tech',
          tag: 'مۆبایل و تەکنەلۆژیا',
          cardTitle: 'مۆبایل و تەکنەلۆژیا',
          contentLabel: 'مۆبایل و تەکنەلۆژیا',
          contentTitle: 'بەستنەوەی خەڵک لە ڕێگەی تەکنەلۆژیاوە',
          contentBody: 'لە ئامێرە مۆبایلەکانەوە تا چارەسەرە دیجیتاڵییەکان، بەشی تەکنەلۆژیای ئێمە خزمەت بە کڕیاران و کۆمپانیاکان دەکات.',
          metrics: [
            { label: 'کارکردنی ئۆنلاین', value: '3' },
            { label: 'هاوبەشەکان', value: '10+' },
            { label: 'پشتیوانی', value: '24/7' }
          ]
        },
        {
          id: 'printing',
          tag: 'چاپەمەنی و فۆتۆکۆپی',
          cardTitle: ' چاپەمەنی و فۆتۆکۆپی ',
          contentLabel: 'چاپ و کۆپیکردن',
          contentTitle: 'چاپی کوالێتی بەرز بۆ هەموو پێداویستییەک',
          contentBody: 'سەنتەری چاپ و کۆپیکردنمان چارەسەری چاپی کوالێتی بەرز بۆ کۆمپانیا و دامەزراوە و تاکەکان دابین دەکات.',
          metrics: [
            { label: 'نووسینگە', value: '1' },
            { label: 'جۆرەکان', value: 'هەموو' },
            { label: 'خێرایی', value: 'خێرا' }
          ]
        },
        {
          id: 'online-trading',
          tag: 'بازرگانی بە ئۆنڵاین',
          cardTitle: 'بازرگانی بە ئۆنڵاین',
          contentLabel: 'بازرگانی ئۆنلاین',
          contentTitle: 'بازرگانی زیرەک لە بازاڕە دیجیتاڵییەکاندا',
          contentBody: 'بەشی بازرگانی بە ئۆنلاینمان 3 کەناڵی بازاڕی دیجیتاڵی کارا بەڕێوەدەبات، کە دەرفەت دەدات بە کڕیاران بۆ دەستڕاگەیشتن بە ئامێرە داراییە جیهانییەکان.',
          metrics: [
            { label: 'کەناڵەکان', value: '3' },
            { label: 'هاوبەشە بانکییەکان', value: '14' },
            { label: 'بەردەوامی', value: '99.9%' }
          ]
        }
      ]
    },
    timelineSectors: {
      eyebrow: "سێکتەرەکانمان",
      headline: {
        diverse: "توانای جۆراوجۆر،",
        unified: "دیدگای",
        vision: "یەکگرتوو."
      },
      subtitle: "چیا گرووپ لە پێنج سێکتەری داینامیکیدا کاردەکات، چارەسەری جێی متمانە پێشکەش دەکات و لە هەر ئاراستەیەکدا کە خزمەتی دەکەین، بەها دەخوڵقێنێت.",
      button: "زیاتر بزانە",
      sectors: [
        {
          id: 'general-trading',
          title: "بازرگانی گشتی",
          desc: "بەشی بازرگانی گشتی ئێمە کۆمەڵێک بەرهەمی جۆراوجۆر لەخۆدەگرێت، کە بازاڕەکانی ناوچەکە بە کوالێتی و متمانە دابین دەکات.",
          items: ['لمة المرجان', 'چیای مەتین', 'چیا ئەمازۆن']
        },
        {
          id: 'money-exchange',
          title: "ئاڵوگۆڕی دراو و سێرڤسی دارایی",
          desc: "چیا گرووپ بەشێکی تایبەت بە ئاڵوگۆڕی دراو بەڕێوەدەبات کە خزمەتگوزارییە داراییە خێرا و سەلامەت و ڕکابەرییەکان پێشکەش دەکات.",
          items: ['خاکی سەروەر', 'چیا', 'چیا گۆڵد', 'لوتکەی چیا', 'بەرزی چیا', 'منفذی دیبگەی هەنگاو']
        },
        {
          id: 'mobile-tech',
          title: "مۆبایل و تەکنەلۆژیا",
          desc: "لە ئامێرە مۆبایلەکانەوە تا چارەسەرە دیجیتاڵییەکان، بەشی تەکنەلۆژیای ئێە هێز دەداتە بەکاربەران و کۆمپانیاکان.",
          items: ['چیا فۆن', 'چیا تێك']
        },
        {
          id: 'printing',
          title: "چاپەمەنی و فۆتۆکۆپی",
          desc: "سەنتەری چاپ و فۆتۆکۆپیمان چارەسەری چاپی کوالێتی بەرز بۆ کۆمپانیا و دامەزراوە و تاکەکان دابین دەکات.",
          items: ['بلو پرێنتینگ']
        },
        {
          id: 'online-trading',
          title: "بازرگانی بە ئۆنڵاین",
          desc: "بەشی بازرگانی ئۆنلاینمان ٣ کەناڵی بازاڕی دیجیتاڵی کارا بەڕێوەدەبات، کە دەرفەت دەدات بە کڕیاران بۆ دەستڕاگەیشتن بە هەلە داراییە جیهانییەکان.",
          items: ['چیا تڕاڤڵ', 'کیڤا لوکژوری']
        }
      ]
    },
    history: {
      eyebrow: "مێژووی ئێمە",
      title: "چیا گرووپ",
      since: "لە ساڵی ٢٠١٩وە",
      description: "نەخشاندنی ئاسۆیەکی نوێ بە دڵسۆزییەکی بێوێنە. بزانە چۆن دیدگای ئێمە بوو بە بناغەیەک بۆ ژێرخانی بەیانی.",
      readMore: "زیاتر بزانە"
    },
    agents: {
      eyebrow: "وەکالەکان",
      headline: {
        excellence: "گەشەپێدانێکی ناوازە،",
        united: "لەسەر بنەمای",
        trust: "متمانە."
      },
      subtitle: "ئێمە وەکالەتی پەسەندکراوی براندە پێشەنگەکانی دارایی و تەکنەلۆژیان، باشترینەکان پێشکەش دەکەین و سەرکەوتن لە ناوچەکەدا بەدی دەهێنین."
    },
    clients: {
      eyebrow: "کڕیارەکانمان",
      headline: {
        trusted: "جێگەی متمانەی براندە",
        worldwide: "پێشەنگەکانی جیهانین."
      },
      subtitle: "ئێمە شانازی دەکەین بە کارکردن لەگەڵ کۆمەڵێکی بەرفراوان لە کڕیاران لە بوارە جیاوازەکاندا. متمانەی ئەوان هاندەرمانە بۆ پێشکەشکردنی باشترینەکان."
    },
    brandQRs: {
      eyebrow: "فۆڵۆی براندەکانمان بکەن",
      title: "کەناڵی فەرمی بڕاندەکانمان",
      description: "سکان بکە بۆ گەڕان لە ناو براندەکانمان و پەیوەندی بەردەوام لە سێکتەرە جیاوازەکانمانەوە.",
      scanToFollow: "سکان بکە بۆ فۆڵۆو",
      banner: {
        title: "یەک گرووپ. چەندین چارەسەر.",
        subtitle: "هەر براندێک لەژێر سایەی چیا گرووپدا کاردەکات بۆ پێشکەشکردنی باشترین خزمەتگوزاری لە بوارەکەیدا.",
        stats: {
          sectors: "٥",
          offices: "١٥ ",
          professionals: "٤٥",
          vision: "١",
          visionLabel: "دیدگای یەکگرتوو",
          sectorsLabel: "سێکتەر",
          officesLabel: "نووسینگە",
          professionalsLabel: "پسپۆڕ"
        }
      }
    },
    location: {
      eyebrow: "بارەگای سەرەکی",
      title: "سەردانی بارەگای سەرەکیمان بکەن",
      description: "لە شوێنێکی ستراتیژی لە هەولێر. وەرن بۆ بینینی داهاتووی بازرگانی و پیشەسازی لە نزیکەوە.",
      hqTitle: "بارەگای هەولێر",
      hqAddress: "شەقامی ڕووناکی، هەولێر ٤٤٠٠١ ، هەرێمی کوردستان ، عێراق",
      email: "ئیمەیڵ",
      phone: "مۆبایل",
      contactBtn: "پەیوەندیمان پێوە بکەن"
    },
    footer: {
      description: "پێشەنگ لە وەبەرهێنانی جۆراوجۆر و بنیاتنانی داهاتوویەکی گەش و بەردەوام بۆ ناوچەکە.",
      company: "کۆمپانیا",
      sectors: "سێکتەرەکان",
      links: {
        about: "دەربارەی ئێمە",
        teams: "تیمەکانمان",
        history: "مێژووەکەمان",
        contact: "پەیوەندیمان پێوە بکەن"
      },
      copyright: "هەموو مافەکان پارێزراون.",
      followUs: "فۆڵۆومان بکەن",
      address: "هەولێر، هەرێمی کوردستان، عێراق"
    },
    contact: {
      title: "با دەست بکەین بە گفتوگۆ",
      description: "ئەگەر پرسیارتان هەیە دەربارەی خزمەتگوزارییەکانمان، یان هەر شتێکی تر، تیمەکەمان ئامادەیە بۆ وەڵامدانەوەی هەموو پرسیارەکانتان.",
      form: {
        fullName: "ناوی تەواو",
        email: "ناونیشانی ئیمەیڵ",
        message: "نامە",
        placeholderName: "ناوی تەواو",
        placeholderEmail: "ناونیشانی ئیمەیڵ",
        placeholderMessage: "چۆن دەتوانین یارمەتیت بدەین؟",
        submit: "ناردنی نامە",
        sending: "خەریکی ناردنە...",
        privacy: "بە ناردنی ئەم نامەیە، تۆ ڕازیت بە سیاسەتی تایبەتمەندی ئێمە.",
        successTitle: "نامەکەت گەیشت",
        successDesc: "سوپاس، {name}! نامەکەت گەیشت و بەم زووانە وەڵامت دەدەینەوە.",
        redirect: "گەڕانەوە بۆ لاپەڕەی سەرەکی لە ماوەی {seconds} چرکەدا"
      },
      info: {
        title: "زانیاری پەیوەندی",
        emailTitle: "ئیمەیڵمان بۆ بنێرە",
        visitTitle: "سەردانی بارەگا بکە",
        callTitle: "پەیوەندیمان پێوە بکە",
        hqErbil: "بارەگای هەولێر"
      }
    },
    newsArchive: {
      title: "هەواڵ و",
      insights: "تێڕوانینەکان",
      description: "هەواڵ و ڕاگەیەندراوە فەرمییەکانی چیا گرووپ؛ تایبەت بە گەشەپێدان، هاوبەشیکردن و دەستکەوتەکانی کۆمپانیا.",
      latest: "دوایین هەواڵەکان",
      allNews: "هەموو هەواڵەکان",
      noPosts: "هیچ هەواڵێک تا ئێستا بڵاونەکراوەتەوە. تکایە دواتر سەردان بکەوە.",
      readTime: "{minutes} خولەک خوێندنەوە",
      author: "چیا گرووپ"
    },
    about: {
      hero: {
        eyebrow: "ڕەگ و ڕیشەمان بدۆزەرەوە",
        title: "دەربارەی ئێمە",
        description: "بە دیدگاوە هەنگاو دەنێین، بە ئەزموون ڕێنمایی دەکرێین. ئێمە کۆمەڵێک سەرکردەی پیشەسازین کە بۆ داڕشتنی داهاتووی بزنسە جیهانییەکان تێدەکۆشین.",
        buttonText: "گەشتەکەمان ببینە"
      },
      nav: {
        title: "لەم لاپەڕەیەدا",
        leadership: "دەستەی خاوەنداریەتی",
        history: "مێژووەکەمان"
      },
      team: {
        title: "دەستەی خاوەنداریەتی",
        description: "ئاشنابە بە سەرکردایەتییە دووربینەکانمان کە ئاراستەی ستراتیژی و نایابی کارەکانمان بەڕێوە دەبەن.",
        members: {
          haji: "حاجی مظفر صراف",
          hajiRole: "سەرۆکی فەخری گروپ",
          ibrahim: "ابراهیم حاجی مظفر",
          ibrahimRole: "سەرۆکی بۆرد و بەڕێوبەری گشتی گروپ",
          bashir: "بشیر الراجح",
          bashirRole: "ئەندامی دەستەی بۆرد و بەڕێوبەری جێبەجێکار",
          ali: "علی نعمان حداد",
          aliRole: "ئەندامی دەستەی بۆرد و بەڕێوبەری مامەڵەکان",
          qasim: "قاسم علی عثمان",
          qasimRole: "بەڕێوبەری ئیدارەی چیا گروپ"
        }
      },
      history: {
        title: "مێژووەکەمان",
        summary: "چیا گروپ : لە ساڵی 2019 دامەزراوە و 7 ساڵە خزمەت دەکەن لە شاری هەولێر لە 5 سێکتەری جیاواز ( بازرگانی گشتی ، ئاڵوگۆڕی دراو و سێرڤسی دارایی ، مۆبایل و تەکنەلۆژیا ، چاپەمەنی و فۆتۆکۆپی ، بازرگانی ئۆنڵاین ) و ئەندامانی پێکهاتوون لە 44 کەس و خاوەنداریەتی 3 کۆمپانیا و 5 نووسینگەی ئاڵوگۆڕی دراو و 1 نووسینگەی چاپەمەنی و فۆتۆکۆپی و 1 منفذ ی صرفی رواتب و 1 مشروع و 1 پێشانگا و 3 کاری ئۆنڵاین و 14 بریکاری بانک و کۆمپانیا دەکەن",
        explore: "کرتە بکە بۆ زیاتر",
        timeline: {
          foundation: {
            title: "کۆمپانیای لمعة المرجان",
            desc: "بازرگانی گشتی - کەرەستەی بیناسازی (دار و داری MDF و مۆبیلیات و ستیل و چیمەنتۆ و کۆنکرێت).",
            content: "کۆمپانیای لمعة المرجان بۆ بازرگانی گشتی ، یەکەم کۆمپانیای سەر بە سێکتەری بازرگانی گشتی چیا گروپە و لە 2019/11/30 دامەزراوە و کار دەکات لە کەرەستەی بیناسازی وەک ( دار و داری MDF و مۆبیلیات و ستیل و فوگە و چیمەنتۆ و کۆنکرێت )"
          },
          mateen: {
            title: "کۆمپانیای چیای مەتین",
            desc: "بازرگانی گشتی - ئامێرە ئەلیکتڕۆنیەکان، کەل و پەلی ناو ماڵ، جل و بەرگ.",
            content: "کۆمپانیای چیای مەتین بۆ بازرگانی گشتی ، دووەم کۆمپانیای سەر بە سێکتەری بازرگانی گشتی چیا گروپە و لە 2026/03/01 دامەزراوە و کار دەکات لە ( ئامێرە ئەلیکتڕۆنیەکان و کەل و پەلی ناو ماڵ و جل و بەرگ و کەل و پەلی بالە)."
          },
          blueprint: {
            title: "نووسینگەی بلو پرێنتینگ",
            desc: "چاپەمەنی و فۆتۆکۆپی - پاسپۆرت، کارتی نیشتیمانی، کاروباری چاپ.",
            content: "نووسینگەی بلو پرێنتینگ بۆ چاپەمەنی و فۆتۆکۆپی سەر بە سێکتەری چاپەمەنی و فۆتۆکۆپی چیا گروپە و لە 2025/10/19 دامەزراوە و کار دەکات لە بواری چاپ کردن و وێنە گرتن و بەڕێوەبردنی رسوماتی دائرەی پاسپۆرت و کارتی نیشتیمانی و حجز کردنی ڕێکەوتی دائرە پاسپۆرت."
          },
          exchange: {
            title: "نووسینگەی چیا",
            desc: "ئاڵوگۆڕی دراو - ناردنی پارە و حساب بانکی.",
            content: "نووسینگەی چیا بۆ ئاڵوگۆڕی دراو ، یەکەم لقی سەر بە سێکتەری ئاڵوگۆڕی دراو و سێرڤسی دارایی چیا گروپە و لە 2021/06/14 لە بۆرسەی کۆن ی شاری هەولێر دامەزراوە و کار دەکات لە بواری ئاڵوگۆڕی هەموو جۆرە دراوێك و ناردنی پارە بۆ هەموو وڵاتانی جیهان بە کاش و حسابی بانکی و ناردنی پارە بۆ هەموو کارتە بانکیە نێوخۆیی و نێودەوڵەتیەکان."
          },
          gold: {
            title: "نووسینگەی چیا گۆڵد",
            desc: "ئاڵوگۆڕی دراو - لقی تورکیا (سلۆپی).",
            content: "نووسینگەی چیا گۆڵد بۆ ئاڵوگۆڕی دراو ، دووەم لقی سەر بە سێکتەری ئاڵوگۆڕی دراو و سێرڤسی دارایی چیا گروپە و لە 2023/08/28 لە ناوچەی سلۆپی شاری شرناغ لە وڵاتی تورکیا دامەزراوە و کار دەکات لە بواری ئاڵوگۆڕی هەموو جۆرە دراوێك و ناردنی پارە بۆ هەموو شارەکان و حساب بانکیەکانی نێو وڵاتی تورکیا."
          },
          hangaw: {
            title: "نووسینگەی هەنگاو",
            desc: "ئاڵوگۆڕی دراو - لقی شەقامی ڕووناکی.",
            content: "نووسینگەی هەنگاو بۆ ئاڵوگۆڕی دراو ، سێیەم لقی سەر بە سێکتەری ئاڵوگۆڕی دراو و سێرڤسی دارایی چیا گروپە و لە 2024/03/21 لە جووت سایدی ڕووناکی شاری هەولێر دامەزراوە و کار دەکات لە بواری ئاڵوگۆڕی هەموو جۆرە دراوێك و ناردنی پارە بۆ هەموو وڵاتانی جیهان بە کاش و حسابی بانکی و ناردنی پارە بۆ هەموو کارتە بانکیە نێوخۆیی و نێودەوڵەتیەکان."
          },
          khaki: {
            title: "کۆمپانیای خاکی سەروەر",
            desc: "ئاڵوگۆڕی دراو - لقی ڕێگای پیرمام.",
            content: "کۆمپانیای خاکی سەروەر بۆ ئاڵوگۆڕی دراو ، چوارەم لقی سەر بە سێکتەری ئاڵوگۆڕی دراو و سێرڤسی دارایی چیا گروپە و لە 2025/02/16 لە ڕێگای پیرمام نێو بەنزینخانەی بی ئێم ٢ لە شاری هەولێر دامەزراوە و کار دەکات لە بواری ئاڵوگۆڕی هەموو جۆرە دراوێك و ناردنی پارە بۆ هەموو وڵاتانی جیهان بە کاش و حسابی بانکی و ناردنی پارە بۆ هەموو کارتە بانکیە نێوخۆیی و نێودەوڵەتیەکان."
          },
          lutkay: {
            title: "نووسینگەی لوتکەی چیا",
            desc: "ئاڵوگۆڕی دراو - لقی عەنکاوە.",
            content: "نووسینگەی لوتکەی چیا بۆ ئاڵوگۆڕی دراو ، پێنجەم لقی سەر بە سێکتەری ئاڵوگۆڕی دراو و سێرڤسی دارایی چیا گروپە و لە 2024/10/20 لە گەڕەکی عەنکاوەی شاری هەولێر دامەزراوە و کار دەکات لە بواری ئاڵوگۆڕی هەموو جۆرە دراوێك و ناردنی پارە بۆ هەموو وڵاتانی جیهان بە کاش و حسابی بانکی و ناردنی پارە بۆ هەموو کارتە بانکیە نێوخۆیی و نێودەوڵەتیەکان."
          },
          barzy: {
            title: "نووسینگەی بەرزی چیا",
            desc: "ئاڵوگۆڕی دراو - لقی سۆران.",
            content: "نووسینگەی بەرزی چیا بۆ ئاڵوگۆڕی دراو ، شەشەم لقی سەر بە سێکتەری ئاڵوگۆڕی دراو و سێرڤسی دارایی چیا گروپە و لە 2025/03/06 لە ئیدارەی سەربەخۆی سۆران لە سەنتەری شار بەرامبەر بازاڕی لەنگە دامەزراوە و کار دەکات لە بواری ئاڵوگۆڕی هەموو جۆرە دراوێك و ناردنی پارە بۆ هەموو وڵاتانی جیهان بە کاش و حسابی بانکی و ناردنی پارە بۆ هەموو کارتە بانکیە نێوخۆیی و نێودەوڵەتیەکان."
          },
          dibaga: {
            title: "منفذ ديبكة ى هەنگاو",
            desc: "صرفی رواتب - سەربازی و مەدەنی.",
            content: "منفذ دیبكة ی هەنگاو بۆ صرفی رواتب سەر بە نووسینگەی هەنگاو لە سێکتەری ئاڵوگۆڕی دراو و سێرڤسی دارایی چیا گروپە و لە 2025/08/01 لە جووت سایدی ڕووناکی لە نێو نووسینگەی هەنگاو دامەزراوە و کارەکانی پێک دێت لە صرفی رواتب عسکری و مدنی و تقاعد و جێبەجێکردنی سێرڤسی ( کی کارد ، سوپەر کی ، ماستەر کارد ) و ناردن و ڕاکێشانی پارە تیایدا."
          },
          amazon: {
            title: "مشروع چیا ئەمازۆن",
            desc: "بازرگانی ئۆنڵاین - کەل و پەلی بالە (تاک و کۆ).",
            content: "مشروع چیا ئەمازۆن بۆ کەل و پەلی بالە بە تاک و بە کۆ سەر بە سێکتەری بازرگانی گشتی چیا گروپە و لە 2026/03/01 لە ڕێگای ڕەواندوز لە ئیدارە سەربەخۆی سۆران دامەزراوە و کارەکانی پێک دێت لە کڕین و فرۆشتنی کەل و پەلی بالە بە تاک و بە کۆ بە پێی سیستەمی شرعی ( حەلال )."
          },
          phone: {
            title: "پێشانگای چیا فۆن",
            desc: "تەکنەلۆژیا - مۆبایل، ئایپاد، لاپتۆپ.",
            content: "پێشانگای چیا فۆن بۆ بازرگانی مۆبایل سەر بە سێکتەری مۆبایل و تەکنەلۆژیای چیا گروپە و لە 2026/03/24 لە سەنتەری شار لە ئیدارەی سەربەخۆی سۆران دامەزراوە و کارەکانی پێک دێت لە کڕین و فرۆشتنی مۆبایل و ئایپاد و لاپتۆپ و کۆمپیتەر و هەموو ئامێرێکی ئەلیکتڕۆنی و ئێکسسوارات."
          },
          travel: {
            title: "کاری چیا تڕاڤل",
            desc: "گەشتوگوزار - تکت، ڤیزە، هۆتێل.",
            content: "چیا تڕاڤل بۆ تکت و ڤیزە سەر بە سێکتەری بازرگانی ئۆنڵاینی چیا گروپە و لە 2020/01/22 دامەزراوە و کارەکانی پێک دێت لە تکت و ڤیزەی گەشتیاری و حجزی هۆتێل و ئۆتۆمبێل و شوێنە گەشتیاریەکان و ڕێنمویی نەخۆش و وەرگێڕی زمان."
          },
          tech: {
            title: "کاری چیا تێك",
            desc: "تەکنەلۆژیا - دانانی سیستەم و کۆمپیتەر.",
            content: "چیا تێك بۆ دانانی سیستەم و کۆمپیتەر سەر بە سێکتەری بازرگانی ئۆنڵاینی چیا گروپە و لە 2025/08/11 دامەزراوە و کارەکانی پێک دێت لە دانانی سیستەمی ژمێریاری و ئامار و ئەرشیف و کارمەند و فرۆشتنی هەموو ئامێرە ئەلیکتڕۆنیەکان"
          },
          luxury: {
            title: "کاری کیڤا لوکژوری",
            desc: "ئێکسسوارات - کاتژمێر، بۆن، ئێکسسواراتی بڕاند.",
            content: "کیڤا لوکژوری بۆ گەوهەر و ئێکسسوارات سەر بە سێکتەری بازرگانی ئۆنڵاینی چیا گروپە و لە 2022/01/20 دامەزراوە و کارەکانی پێک دێت لە بازرگانی گەوهەر و ئێکسسوارات و کاتژمێر و بۆن ی بڕاندی جیهانی."
          }
        }
      }
    }
  }
};

export type Locale = keyof typeof translations;
